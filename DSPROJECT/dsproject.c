...
void editpatient()
{
	...
	char newname[30];
		...
		int f=1,i;
		fp = fopen("patient.txt", "rb");
		printf("enter old patient name:");
		 scanf("%s",oldname);
		 while(fread(&p, sizeof(p), 1, fp) == 1){
		 	 if(strcmp(p.patientname,oldname)==0)
		 	 {
		 	 	printf("\nenter the new patient name");
		 	 	 ...
		 	 	 {
		 	 	 	p.patientname[i]=newname[i];
				 }
		 	 	  fwrite(&p.patientname,sizeof(p.patientname),1,fp);
		 	 	  break;
		 	 }
		 	 printf("updation done...");
        }
